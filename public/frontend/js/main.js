...
function openMobileCategories(element){
    if($(element).hasClass('c-base-1')){
        $(element).removeClass('c-base-1')
        $('.mobile-categories-wrap').removeClass('open')
    } else {
        $(element).addClass('c-base-1')
        $('.mobile-categories-wrap').addClass('open')
        sideMenuClose();
        closeMobileCart();
    }
}
...
function closeMobileCategories(){
    $('.mobile-categories-wrap').removeClass('open')
    $('#mobile-nav-categories').removeClass('c-base-1')
}

function openMobileCart(element){
    if($(element).hasClass('c-base-1')){
        $(element).removeClass('c-base-1')
        $('.mobile-cart-wrap').removeClass('open')
    } else {
        closeMobileCategories();
        sideMenuClose();
        $(element).addClass('c-base-1')
        $('.mobile-cart-wrap').addClass('open')
    }
}

function closeMobileCart(){
    $('.mobile-cart-wrap').removeClass('open')
    $('#mobile-nav-cart').removeClass('c-base-1')
}
...
function sideMenuOpen(e) {
    ...
    if(typeof e == 'string'){
        if($(`#${e}`).hasClass('c-base-1')){
            $(`#${e}`).removeClass('c-base-1')
...
function sideMenuClose() {
    $('#mobile-nav-account').removeClass('c-base-1')
    $(".side-menu-wrap,.side-menu-overlay")
        .removeClass("opacity-1")
        .addClass("opacity-0");
    $(".side-menu")
...
function cardQuantityResponsivenessHandler(className) {
    // $(`${className}-card`).each(function(index, element){
    //     var cardWidth = $(element).width();
    //     if(cardWidth <= 180){
    //         $(`${className}-product-card-qty-wrap`).each(function(i, e){
    //             $(e).addClass('flex-column justify-content-center')
    //             $(e).children().first().addClass('w-100 mb-2')
    //         })
    //     } else {
    //         $(`${className}-product-card-qty-wrap`).each(function(i, e){
    //             $(e).removeClass('flex-column justify-content-center')
    //             $(e).children().first().removeClass('w-100 mb-2')
    //         })
    //     }
    // })
    ...
    $(`${className}-card`).each(function(index, element){
        ...
        if(cardWidth <= 180){
            ...
            $(`${className}-btn-number`).each(function(i, e){
                $(e).addClass('d-flex w-100 h-100 flex-grow-1 align-items-center justify-content-center')
                $(e).parent().addClass('py-2')
                $(e).parent().removeClass('p-2')
                $(e).parent().addClass('align-items-center justify-content-center')
                // $(e).children().first().addClass('w-100 mb-2')
            })

            $(`${className}-input-number`).each(function(x, y){
                $(y).addClass('input-number-no-reset')
            })

        } else {
            
            $(`${className}-btn-number`).each(function(i, e){
                $(e).removeClass('d-flex w-100 h-100 flex-grow-1 align-items-center justify-content-center')
                $(e).parent().addClass('p-2')
                $(e).parent().removeClass('py-2')
                $(e).parent().removeClass('align-items-center justify-content-center')
            })

            $(`${className}-input-number`).each(function(x, y){
                $(y).removeClass('input-number-no-reset')
            })
