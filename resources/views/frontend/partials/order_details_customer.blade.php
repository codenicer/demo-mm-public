...
@php
    ...
    $payment = getPaymentMethodNameByID($order->payment_id);
@endphp
            ...
            <div class="row">
                <div class="col-lg-6">
                    <table class="details-table table">
                        ...
                        <tr>
                            <td class="strong-600" >{{__('Billing address')}}:</td>
                        ...
                        <tbody>
                            @foreach ($order->orderDetails as $key => $orderDetail)
                                <tr>
                                    ...
                                    <td>{{ single_price($orderDetail->item_price) }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
                    ...
                    <table class="table details-table">
                        <tbody>
                            <tr>
                                <th>{{__('Subtotal')}}</th>
                                <td class="text-right">
                                    <span class="strong-600">{{ single_price($order->total_line_items_price) }}</span>
                                </td>
                            </tr>
                            <tr>
                                <th>{{__('Shipping')}}</th>
                                <td class="text-right">
                                    <span class="text-italic">{{ single_price($order->shipping_fee) }}</span>
                                </td>
                            </tr>
                            <tr>
                                <th>{{__('Coupon Discount')}}</th>
                                <td class="text-right">
                                    <span>{{ $order->discount_code }} <em>{{ single_price($order->total_discount) }}</em> </span>
                                </td>
                            </tr>
                            <tr>
