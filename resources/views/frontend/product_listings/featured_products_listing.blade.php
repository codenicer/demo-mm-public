            ...
            <div class="row">
                <div class="col">
                    <ul class="breadcrumb">
                        ...
                        <li><a href="#">{{__('Featured Products')}}</a></li>
            ...
            <div class="row">
                                     ...
                                     @if($cat['count'] > 0)
                                        <div class="single-category">
                                            @if($key == "All products")
                                                ...
                                                href="{{ route('featured_products.product-list') }}"
                                            ...
                                            @else
                                                <a class='w-100 sub-category-name'
                                                href="{{ route('featured_products.list-category', [encrypt($key)]) }}"
                                                style="color:{{$key == $categoryName ? '#eb7c16' : ''}}">
                                                    {{ __($key) }}
                                                    ({{$cat['count']}})</a>
                                            @endif
                ...
                <div class="col-xl-9">
                    <form class="" name="form2"
                          ...
                          action="{{ route('featured_products.list-category-sortby', ['category' => encrypt($categoryName)]) }}"
                          @else
                          action="{{ route('featured_products.list-sortby') }}"
                          @endif
                          method="GET">
                        @csrf
                    </form>
                    <form class="" id="search-form1" action="{{ route('featured_products.list-search', ['search' => $query ? $query : '']) }}" method="GET">
...
@section('script')
    ...
    <script>
        ...
        @if(count($products) > 0)
            ...
            $.post('{{ route('loadmore') }}', {_token:'{{ csrf_token() }}', skip : skip, type: "featured", category_id: "{{isset($categoryId) ? $categoryId : null}}"}, function(data){
                    if(data == 0){
