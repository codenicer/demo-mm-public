@if(count($product_ids) > 0)
    ...
    <label class="col-sm-2 control-label">{{__('Discounts')}}</label>
    <div class="col-sm-9">
    		...
    		<tbody>
                @foreach ($product_ids as $key => $id)
                	@php
                        ...
                        $flash_deal_product = \App\FlashDealProduct::where('flash_deal_id', $flash_deal_id)->where('product_id', $product->product_id)->first();
                	@endphp
                		<tr>
                			<td>
                                <div class="col-sm-3">
