            ...
            <div class="form-group">
                ...
                <div class="col-lg-9">
                    <select name="product_ids[]" class="form-control product_id demo-select2" required>
                        @foreach(\App\Product::where('category_id', $details->category_id)->get() as $key => $product)
                            <option value="{{$product->product_id}}" @if ($details->product_id == $product->product_id)
                                selected
...
<div class="more hide">
    <div class="product-choose">
        <div class="form-group">
           <label class="col-lg-3 control-label">{{__('Category')}}</label>
           <div class="col-lg-9">
              <select class="form-control category_id" name="category_ids[]" onchange="get_products_by_category(this)">
...
<div class="form-group">
    ...
    <div class="col-lg-8">
       <input type="number" min="0" step="1" placeholder="{{__('Quantity')}}" name="coupon_quantity" class="form-control" value="{{ $coupon->units }}" required>
