	...
	<table class="table table-bordered">
		<thead>
			<tr>
				...
				<td class="text-center">
					<label for="" class="control-label">{{__('Campaign Price')}}</label>
				</td>
				<td class="text-center">
...
@foreach ($combinations as $key => $combination)
	...
	@if(strlen($str) > 0)
			<tr>
				...
				<td>
					<input type="text" name="sku_{{ $str }}" value="{{ $sku }}" class="form-control" readonly required>
				</td>
				<td>
					<input type="number" name="price_{{ $str }}" value="{{ $unit_price }}" min="0" step="0.01" class="form-control" required>
				</td>
				<td>
					<input type="number" name="campaign_price_{{ $str }}" value="{{ $campaign_price }}" min="0" step="0.01" class="form-control" required>
				</td>
				<td>
