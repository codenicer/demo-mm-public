...
<div class="row">
    <div class="col-lg-12">
        <a onclick="addHub()" class="btn btn-rounded btn-info pull-right">{{__('Add New Web Hub')}}</a>
    </div>
</div><br>
<div class="panel">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><label class="switch">
                            <input onchange="update_hub_enable(this)" value="{{ $hub->id }}" type="checkbox"
                                <?php if($hub->web_enabled == 1) echo "checked";?>>
                            <span class="slider round"></span></label>
                    </td>
                    <td><label class="switch">
                            <input onchange="update_hub_active(this)" value="{{ $hub->id }}" type="checkbox"
                    ...
                    <td>
                        <div class="btn-group dropdown">
                            ...
                            <ul class="dropdown-menu dropdown-menu-right">
                                <li><a href="#" onclick="edit({{$hub->id}})">{{__('Edit')}}</a></li>
                                <li><a href="#"
                                        onclick="confirm_modal('{{route('web_hubs.destroy', $hub->id)}}');">{{__('Delete')}}</a>
                                </li>
                            </ul>
                        </div>
                    </td>
...
<script>
        ...
        if(this.checked){
            ...
            let hubs = {!!collect($hubs)!!};
                    ...
                    for (let i = 0; i < hubs.length; i++) {
                        options += '<option value="'+ hubs[i].id +'">'+ hubs[i].name +'</option>';
                    }
    ...
    function edit(hub_id){
        $.post('{{ route('web_hubs.edit') }}',{_token:'{{ csrf_token() }}', hub_id:hub_id}, function(data){
                if(data){
                    $("#hubUnifiedId").empty();
                    ...
                    let options = '';
                    for (let i = 0; i < hubs.length; i++) {
                        if(data.unified_hub_id == hubs[i].id){
                            options += '<option value="'+ hubs[i].id +'" selected>'+ hubs[i].name +'</option>';
                        }
                        else{
                            options += '<option value="'+ hubs[i].id +'">'+ hubs[i].name +'</option>';
                        }
    ...
    function saveHub(){
        ...
        if(reportValidity){
            $.post('{{ route('web_hubs.update') }}',{_token:'{{ csrf_token() }}',
                    hub_id: $("#hubId").val(),
    ...
    function update_hub_enable(el){
        ...
        $.post('{{ route('web_hubs.update_enable') }}', {_token:'{{ csrf_token() }}', id:el.value, status:status}, function(data){
          if(data == 1){
            location.reload();
          }
          else{
    ...
    function update_hub_active(el){
        ...
        $.post('{{ route('web_hubs.update_active') }}', {_token:'{{ csrf_token() }}', id:el.value, status:status}, function(data){
          if(data == 1){
            location.reload();
          }
          else{
